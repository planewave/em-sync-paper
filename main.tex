...
A customized entropy estimation algorithm is proposed to achieve fast entropy estimation for the synchronization parameter search.
The synchronization algorithms are implemented and several practical issues are addressed.
Its performance is analyzed in controlled conditions and with realistic measurement data.
Comparing to maximum likelihood based methods, it is shown that entropy minimization based algorithms have superior performance in various communication conditions, especially in channels with non-Gaussianity or non-linearity.
