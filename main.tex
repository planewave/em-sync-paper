...
Hence, at the output of a matched filter, the signal sampled at the ideal instant will have maximum signal to noise ratio (SNR) and no intersymbol interference~(ISI)~\cite{mengali1997synchronization}.
...
In contrast, various DA frequency offset estimation algorithms attempt to maximize the inner product between the  training sequence and the data samples.
Specifically, the orthogonal frequency division multiplexing (OFDM) CFO estimation is essential because the CFO results in loss of orthogonality between the OFDM carriers.
...
Note that it has been shown in~\cite{Pedzisz2006} that the CFO of phase-shift keying (PSK) modulated signals can be recovered by minimizing the entropy of the instantaneous phase probability density function.
...
where a kernel function is used to estimate the quadratic R\'enyi entropy~\cite{Santamaria2002,Huang2008}.
...
Then, the application of the EM criterion to symbol timing using the eye diagram entropy is explained in Section \ref{sec:eye_entp}, while the carrier frequency recovery using constellation diagram entropy is explained in Section \ref{sec:const_entp}.
...
The received passband signal goes through a down converter and matched filter.
...
Also, by applying symbol timing recovery first, the carrier recovery has less computational burden because it operates at a lower rate.
...
In the next two subsections, the entropy is used as a metric to evaluate the eye diagram and constellation diagram.
...
For example, a given modulation scheme with an alphabet size of \(M\) is implemented to transmit i.i.d. random data. 
...
At the receiver, the signal waveform that is sampled at the perfect symbol timing instant (in the middle of the eye diagram) can only be distributed equally within \(M\) possible symbols.
...
If we examine the signal distribution at timing instants that deviate from the middle of the eye, 
interference from adjacent symbols will distort the amplitudes.
...
the maximum eye diagram entropy will be \(N_{span}\) times of the amount of information carried by each symbol, which can be presented as
...
The eye diagram entropy can be seen as an indicator of the ISI that is introduced by the pulse shaping filter.
...
Only the recovering a CFO that is much smaller than the symbol rate will be discussed in this section, since for a large CFO, a preceding coarse carrier recovery is usually required.
...
A common technique is to use the histogram for probability estimation as shown in Fig.~\ref{fig:eye_ent}.
...
However, an inefficient histogram based probability estimation is still need to obtain a relative \(H_{R2 }\) in~(\ref{eq:renyi2}).
...
It is also known as Parzen's window \cite{Parzen1962} as one of the most well-known non-parametric approaches to estimate the underlying PDF of samples. 
...
Specifically, given a set of \(N\) samples, \(i=1, \dots, N\), the sample PDF at an observation \(\xi\) can be estimated by
\begin{equation}
...
By exploring the higher order statistics of the signal, entropy minimization is expected to have improved performance. 
